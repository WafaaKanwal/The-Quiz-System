...
const introduction = () => {
  ...
  console.log(
    chalk.yellowBright.bold("**      🌟  WELCOME TO THE QUIZ! 🌟         **")
  );
  console.log(
    chalk.yellowBright.bold("**     Test your TypeScript knowledge!      **")
  );
  console.log(
    chalk.yellowBright.bold("**                                          **")
  );
...
const checkAnswer = (question: Question, userAnswer: string): boolean => {
  if (userAnswer === question.correctAnswer) {
    ...
    console.log(
      chalk.redBright(
        `❌ Incorrect! The right answer is ${chalk.cyanBright(
...
const main = async () => {
  ...
  results.forEach((result, index) => {
    console.log(
      `${index + 1}. ${result.question} - ${
        result.correct ? chalk.green("Correct") : chalk.red("Incorrect")
      }`
    );
  });
  ...
  console.log(
    ...
    │ 📝  Your total score is: ${chalk.cyanBright.bold(
      score
    )} / 50             │
    ╰──────────────────────────────────────────────╯
 `)
